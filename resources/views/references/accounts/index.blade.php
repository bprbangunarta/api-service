    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list">
                    @if (request('search'))
                    <a href="{{ route('ref-account.index') }}" class="btn btn-icon btn-danger" style="margin-right: -5px;">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body border-bottom py-3">
                        <form action="{{ route('ref-account.index') }}" method="GET">
                            <div class="d-flex">
                    ...
                    <div class="table-responsive">
                        <table class="table card-table table-vcenter text-nowrap datatable">
                            ...
                            <tbody>
                                @forelse ($data['accounts'] as $index => $item)
                                <tr class="text-reset">
                                    <td><a href="{{ route('ref-account.show', Crypt::encryptString($item->id)) }}" class="text-reset d-block">{{ $item->code }}</a></td>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ $item->balance }}</td>
                                </tr>
                                @empty
                    ...
                    <!-- pagination -->
                    {{ $data['accounts']->withQueryString()->onEachSide(0)->links('layouts.paging') }}
                </div>
            </div>
        </div>
    </div>
